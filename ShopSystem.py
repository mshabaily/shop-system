...
class RowContainer(Frame):
    ...
    def delete(self):
        ...
        self.destroy()
    def popup(self):
        self.menu.tk_popup(window.winfo_pointerx(), window.winfo_pointery())
    ...
    for array in arrays:
        ...
        for object in array:
            ...
            for attribute in attributes:
                ...
                file.write(",")
            if array.index(object) != len(array):
                file.write("\n")
        file.close()
...
def formatTable(objects):
    ...
    tableContainer = Frame(mainCanvas)
    mainCanvas.create_window((0,0), width = mainCanvas.winfo_reqwidth(), window = tableContainer, anchor = "nw")
    for row, item in enumerate(objects):
        rowContainer = RowContainer(tableContainer, background="white")
        for column, attribute in enumerate(attributes):
            rowContainer.grid_columnconfigure(index=column, weight = 1)
            field = Field(rowContainer, item, attribute)
